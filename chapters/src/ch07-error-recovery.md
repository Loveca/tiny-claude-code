...
不要在 agent loop 里到处写 retry。让 `ErrorHandler` 接收一个 callable，由它决定是否等待、重试、切模型或放弃。

### 第二步：按错误类型分支

429、529、token limit、普通异常不能混在同一个 except 里。错误类型越明确，恢复动作越可控。

### 第三步：把循环保护放在 agent loop

API 重试只能处理请求失败，不能处理模型无限调用工具。最大轮次必须放在 agent loop 层。

### 第四步：把 malformed tool_use 变成观察

协议错误不要让 Python 崩溃。把错误包装成 `tool_result` 回给模型，模型才有机会自我修正。

## 测试讲解

本章测试要避免真的等待很多秒。退避函数和 sleep 最好可注入或可 mock，这样可以验证“会等待多久”，而不是让测试真的睡眠。

fallback model 的测试也要检查恢复后模型状态是否符合预期。可以选择临时切换后恢复，也可以保持备用模型，但行为必须明确。

## 常见错误

### 无限重试

所有恢复策略都必须有上限。没有上限的 retry 会把临时故障变成卡死。

### 把所有异常都当成可恢复

认证失败、参数错误、代码 bug 通常不应该重试。重试只适合临时性错误。

### 混淆输入和输出 token limit

输出 token 不够可以提升 `max_tokens`；输入上下文过大则需要 ch08 的上下文管理。二者不是同一个问题。
